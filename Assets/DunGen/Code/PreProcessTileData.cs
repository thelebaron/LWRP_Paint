...
namespace DunGen
{
    public sealed class PreProcessTileData
    {
		...
		public PreProcessTileData(GameObject prefab, bool ignoreSpriteRendererBounds, Vector3 upVector)
        {
			...
			bounds.size -= Vector3.one * 0.01f; // We need to shrink the actual bounds by a small amount to avoid tiles from colliding when their doorways are perfectly aligned
